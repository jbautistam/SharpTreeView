    ...
    partial class SharpTreeNode
	{
		...
		private static void RebalanceUntilRoot(SharpTreeNode pos)
		{
			...
			if (newRoot != pos && pos.TreeFlattener != null) {
				...
				newRoot.TreeFlattener._root = newRoot;
			}
		...
		private void ReplaceWith(SharpTreeNode node)
		{
			...
			} else 
			{
				...
				if (TreeFlattener != null) 
				{
					...
					TreeFlattener = null;
					node.TreeFlattener._root = node;
				}
			}
		}
		
		/// <summary>
		///		Colección de elementos del árbol en una estructura plana
		/// </summary>
		internal TreeFlattenerCollection? TreeFlattener { get; set; }
	}
}
